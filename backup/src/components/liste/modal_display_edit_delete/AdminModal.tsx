...
import { _editAdmin } from '../../../redux/actions/admin.action'
    ...
    return (
        !seeModalDisplayEditDelete ? <></> :
            <>
                        ...
                        <div className='modal_content'>
                            <div className='display_information_container'>
                                ...
                                <div className='display_information'>
                                    ...
                                    <span className='value'> {row?.name?.length < 30 ? row?.name : row?.name?.substring(0, 30) + '...'} </span>
                                </div>

                                <div className='display_information'>
                                    <span className='title'>Email</span>
                                    <span className='value'> {row?.email?.length < 30 ? row?.email : row?.email?.substring(0, 30) + '...'} </span>
                                </div>

                                <div className='display_information'>
                                    <span className='title'>Rôle</span>
                                    <span className='value'> {row?.role === 'super-admin' ? 'Super administrateur' : 'Administrateur'} </span>
                                </div>
                            </div>
                        </div>
                    </Modal>
                ...
                {type === 'supprimer' &&
                    <Modal title={'Suppression de l\'administrateur'} setOpenAddModal={setSeeModalDisplayEditDelete}>
                        <div className='modal_content'>
